...
- **이미지 최적화 / 노션 파일 URL 만료** (2026-07-30 실측으로 구체화):
  - 노션 업로드 파일은 `prod-files-secure.s3.us-west-2.amazonaws.com` 의 **서명 URL**이고 `X-Amz-Expires=3600` — **유효기간 1시간**이다. 응답의 `expiry_time` 으로도 확인된다.
  - ⚠️ 페이지 `revalidate` 도 3600 이라 **두 주기가 겹친다.** 캐시된 HTML 이 만료 직전 URL 을 들고 있으면 재생성 전에 방문한 사용자에게 이미지가 깨져 보인다.
  - 대응: `next/image` 로 프록시하면 Next 가 최적화 이미지를 자체 캐시해 서명 URL 만료와 분리된다(Phase 5 에서 처리, 권장). 대안은 `revalidate` 를 만료보다 짧게 낮추는 것이지만 노션 호출이 늘어난다.
  - 이 특성 때문에 썸네일/아바타 URL 을 DB 나 상수에 **영구 저장하면 안 된다.** 장기적으로 자체 스토리지 미러링은 P2.
...
- 이력서 PDF 에 자격증/교육 이력 섹션을 추가할지 — 현재는 프로필 + 경력만 포함한다.

**해소된 항목 (2026-07-30)**
- ~~프로필을 DB 로 관리할지 일반 페이지로 할지~~ → **DB(1행)** 확정. 매퍼·조회 코드가 경력/프로젝트와 같은 패턴이 되고 확장도 쉽다. 구현 완료.
- ~~프로젝트 정렬 방식~~ → **`정렬 순서`(order) 우선, 값이 없으면 기간 최신순 폴백**. 노션 정렬로는 "빈 값을 뒤로"를 표현하기 어려워 조회 후 JS 에서 정렬한다(`fetchProjects`).
- ~~재생성 주기~~ → **1시간(3600초)** 확정. Next 가 정적 분석하는 값이라 각 페이지·라우트에 리터럴로 선언한다.
- ~~이력서 PDF 포함 범위~~ → **프로필 + 경력**. 프로젝트 요약은 FR-5 범위가 아니라 미포함.
- ~~프로젝트 필터링·검색 기능~~ → **범위 제외**. FR 표에 없는 제안 기능이고 공개 프로젝트가 3건뿐이어서 UI 가 방해가 된다. 10건 이상으로 늘면 재검토.
